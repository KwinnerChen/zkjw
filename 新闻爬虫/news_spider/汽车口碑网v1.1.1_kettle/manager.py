...
def storage_process(db, q_storage_task, table_name):  # db链接到已定数据库
    # logger = log('storage_log.txt')
    while True:
        r = q_storage_task.get()
        if r is None:
            break
        try:
            db.save(r, table_name)
            print(r['URL'], '入库成功！')
        except:
            n = 0
            while True:
                print(r['URL'], '存储时出错，5秒后重试...')
                time.sleep(5)
                try:
                    db.save(r, table_name)
                    print(r['URL'], '入库成功！')
