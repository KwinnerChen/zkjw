...
class IPPool():
    ...
    #default_url = 'http://www.66ip.cn/nmtq.php?getnum=300&isp=0&anonymoustype=0&start=&ports=&export=&ipaddress=&area=1&proxytype=0&api=66ip'
    default_url = 'http://www.89ip.cn/tqdl.html?api=1&num=300&port=&address=&isp='
    def __init__(self, url = default_url):
        self.headers = {
            'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:62.0) Gecko/20100101 Firefox/62.0',
            'Host':urlsplit(self.default_url).hostname,
        }
        self.url = url
    ...
    def get_ip(self):
        ...
        elif os.path.isfile(self.file_path) and (os.path.getsize(self.file_path) > 2) and (int(time.time()-os.path.getmtime(self.file_path))<1800):
            with open(self.file_path, 'r') as f:
                ips_list = f.readlines()
                return {'http':'http://%s' % random.choice(ips_list).strip()}
        else:
