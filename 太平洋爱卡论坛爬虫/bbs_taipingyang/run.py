...
def log_task_port(spider_name, port):
    with open('task_port_log.log', 'a') as f:
...
def task_num_check(q_task, q_reault, q_storage, q_proxy, logger, thread_list):
    while True:
        ...
        proxy_num = q_proxy.qsize() if q_proxy else 0
...
if __name__ == '__main__':
    ...
    # 启动各线程，监听任务
    download_threading(downloader, q_task, q_reault, d_thread_num, thread_list, logger, delay, proxies_pool=q_proxy, timeout=timeout)
