...
def download_worker(downloader, thread_name, q_task, q_result, logger, delay, proxies_pool, uapool, kwargs):
    while True:
        if not q_task.empty():
            task = q_task.get()
            if task is None:
            ...
            try:
                logger.info('\n下载线程%s正在下载：%s\n' % (thread_name, task))
                resp = download(downloader, task, delay, proxies_pool, uapool, **kwargs)
                erro = '响应对象为空！'
            except:
                resp, erro = download_meet_erro(downloader, task, delay, logger, proxies_pool, uapool, **kwargs)
            if resp:
                task.response = resp
                q_result.put(task)
                logger.info('\n加入到中间列队：%s\n' % task)
...
def download_meet_erro(downloader, task, delay, logger, proxies_pool, uapool, **kwargs):
    try:
        logger.info('\n下载：%s 出错，重试中...\n' % task)
        resp = download(downloader, task, delay, proxies_pool, uapool, **kwargs)
        erro = ''
...
def download(downloader, task, delay, proxies_pool, uapool, **kwargs):
    ...
    if url:
        if method == 'get':
            time.sleep(delay)
            resp = downloader.get(url, proxies_pool=proxies_pool, uapool=uapool, **kwargs)     
        elif method == 'post':
            time.sleep(delay)
            resp = downloader.post(url, proxies_pool=proxies_pool, uapool=uapool, **kwargs)
    else:
        resp = None
    return resp


def download_threading(downloader, q_task, q_result, thread_num, thread_list, logger, delay, uapool, proxies_pool=None, **kwargs):
    for i in range(thread_num):
        thread_name = 'download_threading_%s' % i
        t = Thread(name=thread_name, target=download_worker, args=(downloader, thread_name, q_task, q_result, logger, delay, proxies_pool, uapool, kwargs))
        thread_list.append(t)
        t.start()
        logger.warning(t.name + ' 启动！')
