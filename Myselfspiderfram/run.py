...
def task_num_check(q_task, q_reault, q_storage, logger, thread_list):
    while True:
        if not thread_list:
            break
        ...
        logger.info('\n任务列队长度：%s  中间列队长度：%s  存储列队长度：%s\n' % (task_num, reault_num, storage_num))
        time.sleep(5)

def check_thread(q_task, q_reault, q_storage, logger, thread_list):
    t = Thread(target=task_num_check, args=(q_task, q_reault, q_storage, logger, thread_list))
    t.start()
...
if __name__ == '__main__':
    ...
    check_thread(q_task, q_reault, q_storage, logger, thread_list)
